...
Rather than using AST for traversal, the system utilise a recursive language aware splitting strategy. This approach breaks down code files by identifying structural boundariees based on the programming language syntax. The algorithm is outlined in Algorithm \ref{alg:recursive_splitting}.
...
\Else{
    $Separators \gets \{ \texttt{"\char`\\ nclass "}, \texttt{"\char`\\ ndef "}, \texttt{"\char`\\ n\char`\\ n"}, \texttt{"\char`\\ n"}, \texttt{" "} \}$\;
    $Splitter \gets$ \text{InitialiseRecursiveSplitter}($Separators, S, O$)\;
}
$Documents \gets Splitter.\text{createDocuments}(C)$\;
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        {\small
        \includegraphics[width=6in]{fig/fig12.png}
        \caption{\textit{Example of Multistep Planning by Search Architect Persona}}
        \label{fig:planning_example}
        }
    \end{figure}
    \item \textbf{Gap Analyser Persona:} To give the LLM autonomy in exploring the codebase, this persona acts as a quality control layer to identify gaps in the retreived context. If there are any missing pieces of information, it will trigger an additional search iteration with refined queries. This iterative process continues until sufficient context is gathered to answer the user's query accurately. Figure \ref{fig:gap_analysis_example} illustrates an example of gap analysis performed by this persona.
    \begin{figure}[H]
        \centering
        {\small
        \includegraphics[width=6in]{fig/fig13.png}
        \caption{\textit{Example of Gap Analysis by Gap Analyser Persona}}
        \label{fig:gap_analysis_example}
        }
    \end{figure}
    \item \textbf{Graph Architect Persona:} This persona is responsible for translating the retrieved context into a graph object required for React Flow. It identifies the relevant code entities and their relationships, ensuring that the graph accurately represents the code structure and logic flow.
\end{itemize}
...
Raw user queries are usually ambiguous and require refinement. The pipeline first rephrases the query into technical search queries that align with the codebase's terminology and structure. This is achieved using the Search Architect persona described earlier.
...
\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=6in]{fig/fig14.png}
...
\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=6in]{fig/fig15.png}
...
\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=6in]{fig/fig16.png}
...
\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=6in]{fig/fig17.png}
...
\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=6in]{fig/fig18.png}
...
To overcome GitHub API rate limits, the system implements a \textbf{Streaming Tarball Strategy}. By fetching a single compressed \texttt{.tar.gz} archive, the system reduces network calls by over 90\% and processes repository data entirely in-memory.

\subsection{Real-Time Graph Updates}
To ensure a smooth user experience, the system utilizes Server-Sent Events (SSE) to stream graph data. This allows the React Flow canvas to begin rendering nodes as they are discovered by the RAG agent, rather than waiting for the entire process to complete.
