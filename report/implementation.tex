...
\begin{enumerate}
    \item \textbf{Requirement Analysis:} Pain points for new developers navigating unfamiliar codebases were identified to gather initial requirements.
    \item \textbf{Prototyping:} To validate core concepts, early prototypes of the search and Generative UI systems were built. For example, a simple weather card UI was created to test the Generative UI's capabilities, as shown in \textit{Figure~\ref{fig:dynamic_card_generation_weather_app}}.
    \begin{figure}[H]
        \centering
        {\small
    ...
    \end{figure}
        \item \textbf{Incremental Development:} The search module, code graph analysis, and RAG pipeline were identified as priority features. Development of these features was done incrementally to gather early feedback.
    \item \textbf{Integrating User Feedback:} User testing feedback was regularly incorporated to improve the CodeOrient user experience.
    \item \textbf{Final Testing and Optimisation:} The system underwent rigorous testing and automated deployment via CI/CD pipelines to ensure the performance and reliability of production release.
\end{enumerate}

\subsection{Version Control and Branching Strategy}
This project utilised Git and GitHub for version control. Feature-branching was used to isolate the development of core features. This ensured that the main branch remained stable and protected for user testing. As for deployment, it was automated through CI/CD pipelines and was deployed to Vercel. \textit{Figure~\ref{fig:git_branching_strategy}} illustrates the branching strategy which, squashes feature branches into the main branch after code reviews and testing.
\begin{figure}[H]
    \centering
    {\small
...
\end{itemize}
CodeOrient uses this retrieval method to find code snippets that match the keywords in the user query. For example, if a user searches for an exact function name, BM25 will prioritise documents that contain that term.
...
Rather than using AST for traversal, the system utilises a recursive language-aware splitting strategy. This approach splits the code into smaller chunks while maintaining the programming language's nuances, syntax and structure. This strategy is outlined in \textit{Algorithm~\ref{alg:recursive_splitting}}.
...
\Return{$K$}\;
\caption{\textit{Language-Aware Recursive Code Splitting Algorithm}}
\label{alg:recursive_splitting}
\end{algorithm}

\subsubsection{Graph Construction Algorithm}
As React Flow requires a structured object to render the graph, the extracted entities and their relationships are mapped to a JSON object. In \textit{Listing~\ref{lst:graph_metadata_schema}}, the code entities are represented as nodes and their relationships as edges. 
\begin{listing}[H]
...
\end{minted}
\caption{\textit{TypeScript interfaces for CodeOrient graph entities.}}
...
\begin{itemize}
    ...
    \end{figure}
    \item \textbf{Gap Analyser Persona:} To give the LLM autonomy in exploring the codebase, this persona acts as a quality control layer to identify gaps in the retrieved context. An additional search iteration with refined queries will be triggered if the context is deemed insufficient. This iterative process continues until the LLM can accurately answer the user's query. \textit{Figure~\ref{fig:gap_analysis_example}} illustrates an example of the LLM identifying a need for additional context.
    \begin{figure}[H]
        \centering
        {\small
...
User queries are usually ambiguous and may include typos. To address this, the pipeline first rephrases the query into technical search queries (using the Search Architect persona) that align with the codebase's terminology and structure. 
...
The top-ranked code chunks are assembled and wrapped in XML-style tags containing metadata for the LLM to accurately reference the sources during the response phase. An example of the assembled context is shown in \textit{Listing~\ref{lst:assembled_context_example}}.
\begin{listing}[H]
...
\end{minted}
\caption{\textit{Example of Assembled Context with Source Metadata.}}
\label{lst:assembled_context_example}
\end{listing}

\subsubsection{Citation Extraction and Grounding}
To increase the credibility of the generated answer, all sources retrieved from the RAG pipeline are shown to the user in \textit{Figure~\ref{fig:rag_sources_example}}. To further reduce hallucination in the response, the LLM is prompted to provide inline citations using markdown format \texttt{[file\_path](link\_to\_source\_code)}. The frontend parses these citations to create clickable links that direct users to the exact source code locations. An example of inline citations is shown in \textit{Figure~\ref{fig:inline_citations_example}}.

\begin{figure}[H]
    \centering
    {\small
    \includegraphics[width=5.5in]{fig/fig14.png}
    \caption{\textit{Example of sources retrieved from the RAG pipeline}}
...
\end{itemize}
The complete Prisma schema is provided in \textit{Listing~\ref{lst:database_schema}}.
...
// Conversational State + Generative UI Support
model Chat {
  id        String    @id @default(cuid())
  title     String
  messages  Message[]
...
model Part {
  ...
  tool_visualiseCodeGraph_output Json?
  data_codeGraph            Json?
...
\end{minted}
\begin{center}
    \captionof{listing}{\textit{Prisma Database Schema.}}
    \label{lst:database_schema}
\end{center}

\section{UI/UX Enhancements}
To further improve the user experience in CodeOrient, several additional features were implemented:
\subsection{Account Dashboard}
The account dashboard allows users to monitor any activity related to their account. As shown in \textit{Figure~\ref{fig:account_dashboard}}, users can view statistics such as:
\begin{itemize}
    \item Recent Activities
    \item Usage statistics (e.g., number of lifetime searches, total repositories analysed over time)
\end{itemize}
...
As collaboration is a key aspect of software development, CodeOrient supports the sharing of conversations via unique read-only links. \textit{Figure~\ref{fig:sharing_conversation_link}} demonstrates the shared chat view where authorised users can view the conversation history in read-only mode.
\begin{figure}[H]
    \centering
    {\small
...
LLMs can take several seconds to generate responses, leading to a suboptimal user experience. Hence, Server-Sent Events (SSE) is used to stream the graph data incrementally, which allows React Flow to render an empty canvas initially and progressively add nodes and edges after the generation has completed.
