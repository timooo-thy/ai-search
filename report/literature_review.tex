...
The figure below showcases how a query is transformed into an embedding vector which shares the same embedding space as code snippets. Although this technique does not replace traditional code search, it complements traditional methods that often miss semantically relevant results.
...
Among many code visualisation libraries, React Flow stands out for its interactivity and ease of integration with React applications \cite{jovanov2025}. React Flow allows developers to explore the relationships between entities. Rather than traditional large static diagrams, developers can pan and hover over nodes to reveal additional information, which is cruicial for understanding  complex codebases.
