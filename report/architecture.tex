...
A multi-stage pipeline optimised for speed and quality of responses is built to handle user interactions. As shown in \textit{Figure~\ref{fig:data_flow_architecture}}, it illustrates the data flow, split into two main phases: \textit{Code Indexing Phase} and \textit{Query Response Phase}.
...
For vector-based retrieval to work, code indexing for the required code repository is performed as a prerequisite step. This phase parses the source code files, chunks them, and generates vector embeddings to be stored in a vector database. To improve the recall, docstrings and comments are extracted as well. In cases where indexing is not performed, CodeOrient falls back to searching the codebase directly via GitHub Search API during query time.
...
\begin{itemize}
    ...
    \item \textbf{Styling:} Tailwind CSS \& Shadcn UI are utilised to design the user interface, providing a consistent and responsive design system.
\end{itemize}

\subsection{User Interface Design}
This section discusses the key design principles and layout of the chat page of CodeOrient application. \textit{Figure~\ref{fig:chat_page_layout}} showcases the chat page layout, which features a split-pane design that prioritises the chat interface with the AI assistant and the source code.
\begin{figure}[H]
    \centering
    {\small
