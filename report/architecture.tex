...
This chapter details the system architecture of CodeOrient. It will provide a high-level overview of the core components, data flow architecture, frontend and backend design, data pipeline, and technical stack summary.

\subsection{Core Components}
The design of CodeOrient is composed of four main components:
\begin{itemize}
    \item \textbf{User Interface:} Built with Next.js and React Flow to provide an interactive experience between the user and the AI assistant.
    ...
    \item \textbf{Code Search Engine:} A hybrid search engine that combines semantic search with keyword-based retrieval to find relevant code snippets, falling back to the GitHub Search API when necessary.
...
Code indexing is a prerequisite step that ingests the required code repository for vector-based retrieval. This phase parses the source code files, chunks them, and generates vector embeddings for storage in a vector database. To improve recall, docstrings and comments are also extracted. In cases where indexing is not performed, CodeOrient falls back to searching the codebase directly via GitHub Search API during query time.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Query Understanding and Planning:} The LLM first understands and rephrases the user's query. This step fixes potential errors, such as typos and ambiguous terms. If there is insufficient context, the LLM will prompt the user for more information instead. Next, the LLM generates a plan to decompose the query into sub-tasks, generating up to 3 sub-queries for retrieval.
...
\begin{itemize}
    \item \textbf{Ingestion Service:} Handles the ingestion of code repositories from GitHub API asynchronously through background jobs, allowing the frontend to remain responsive.
    ...
    \item \textbf{Persistence and Cache Service:} Prisma, a dedicated Object Relational Mapping (ORM) service, is used to interact with PostgreSQL for persistence of chat history and a Redis client to interact with Upstash Redis for caching frequent queries.
\end{itemize}
...
This section discusses the key design principles and layout of the chat page of the CodeOrient application. \textit{Figure~\ref{fig:chat_page_layout}} showcases the chat page layout, which features a split-pane design that prioritises the chat interface with the AI assistant and the source code.
\begin{figure}[H]
    \centering
    {\small
...
Static architecture diagrams are often high-level and extremely abstracted, making it difficult for users to zoom in on a specific module. To address this, CodeOrient integrates LLMs and React Flow to visualise a subset of the code repository as an interactive graph. The core components of a graph visualisation include:
...
Standard LLM responses are limited to text or markdown outputs. Often, this is insufficient to convey complex code structures or relationships. To address this, CodeOrient adopted a Generative UI architecture in which the LLM generates structured JSON objects that render as functional React components. 

Based on the user's query, the LLM autonomously decides which component to generate. For example, if the user asks about their list of repositories, the LLM would generate a ``Repository Card" component. To simulate a smoother user experience, the component data is streamed to the UI, allowing the card to populate incrementally and reducing perceived latency.

\section{Backend Architecture}
\subsection{API Design}
A RESTful API design pattern is adopted to achieve a clear separation of concerns among backend services. Each endpoint is implemented as a serverless function, allowing each service to scale independently based on demand. The API is divided into four primary services:
\begin{itemize}
...
Creating an autonomous RAG pipeline enables the LLM to move beyond simple search-and-retrieval patterns. Instead, the LLM follows a more complex reasoning process:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item The system utilises LangChain to extract code chunks from the archive in memory, speeding up the ingestion process.
...
Instead of indexing a single file as a single document, the engine splits each code file into smaller ``logical" chunks, ensuring each chunk maintains coherence during retrieval.
\begin{enumerate}
    ...
    \item Each chunk is limited to 1,500 tokens with a chunk overlap of 200 tokens. This overlap is key to maintaining context across chunk boundaries, ensuring that related code segments are not lost during retrieval.
\end{enumerate}

\subsection{Metadata Extraction}
To provide a richer context during retrieval, each code chunk includes additional metadata fields extracted during parsing.
\begin{enumerate}
...
\begin{enumerate}
    \item The system uses Prisma ORM and PostgresSQL to track the real-time progress of the indexing lifecycle of each repository to the user (e.g., \texttt{CLONING}~$\rightarrow$~\texttt{PARSING}~$\rightarrow$ \allowbreak \texttt{INDEXING}~$\rightarrow$~\texttt{COMPLETED}). In the event of a network interruption, this serves as a checkpoint to resume indexing.
    \item To optimise throughput, code chunks are converted into embeddings using the \texttt{bge-large-en-v1.5} model and upserted into Upstash Vector in batches of 100.
\end{enumerate}
