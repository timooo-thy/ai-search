...
CodeOrient handles user interaction througha multi-stage pipeline that is optimised for speed and quality of responses. The lifecycle is split into two phases: \textit{Code Indexing Cycle} and \textit{Query-Response Cycle}.
...
\begin{enumerate}
    ...
    \end{itemize}
    \item \textbf{Citation-Grounded Response:} Once sufficient context is gathered, the LLM generates a structured JSON object that contains the required nodes, edges, code snippets (citations) and explanation required for visualisation and response generation.
...
This section discusses the key design principles and layout of the chat page of CodeOrient application. The page features a split-pane design that prioritise the chat interface with the AI assistant and the source code.
\begin{figure}[H]
    \centering
    {\small
...
\begin{enumerate}
    ...
    \item \textbf{Ranking Mechanism:} Retrieved code snippets are ranked using Distributed-Based Score Fusion (DBSF), which combines results from dense and sparse indexes. The normalised score is computed as:
    \[
...
\begin{enumerate}
    ...
    \item \textbf{In-memory Extraction:} Using the \texttt{tar} and \texttt{zlib} libraries, the system decompressed and parses files in-memory without writing to disk. This reduces I/O overhead and speeds up processing.
...
\begin{enumerate}
    \item \textbf{Entity-Based Parsing}: The parser isolates code blocks such as standalone functions, classes, or React components as an individual entity type. Each type is assigned a chunk size limit to optimise retrieval:
    \begin{table}[H]
        \centering
        \begin{tabular}{@{}lc>{\raggedright\arraybackslash}p{5.5cm}@{}}
            \toprule
            \textbf{Entity Type} & \textbf{Max Character Limit} & \textbf{Rationale} \\
            \midrule
...
\begin{enumerate}
    ...
    \item \textbf{Dependency Mapping:} The system extracts \texttt{imports} and \texttt{calledFunctions}. This enables the LLM to understand the relationships between different code entities during reasoning, which increases the accuracy of generated responses.
...
\begin{enumerate}
    \item \textbf{State Persistence:} The system uses Prisma ORM and PostgereSQL to track the indexing lifecycle of each repository (e.g., \texttt{CLONING}~$\rightarrow$~\texttt{PARSING}~$\rightarrow$~\texttt{INDEXING}~$\rightarrow$~\texttt{`CO'MPLETED}). This allows for resumable indexing in case of interruptions. Additionally, it allows the UI to show real-time progress updates to the user.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lc>{\raggedright\arraybackslash}p{8.5cm}@{}}
        ...
        \addlinespace
        \textbf{LLM Provider} & OpenAI & Offers state-of-the-art models with robust capabilities in code understanding and multi-turn reasoning. \\
        \addlinespace
